...
const activeStatuses = (await AppDataSource.manager.find(Status)).filter(status => status.Status !== 'Dropped');
        ...
        if (!toon) {
            ...
            if (status === 'Main') {
                ...
                for (const main of currentMains) {
                    await AppDataSource.manager.update(ActiveToons, { DiscordId: discordId, Name: main.Name }, { Status: 'Alt' });
                }
            }
            else {
                const currentMains = await AppDataSource.manager.find(ActiveToons, {
                ...
                if (currentMains.length <= 1 && toon.Status === 'Main') {
                    throw new Error(`:x: Cannot change \`${name}\` to \`${status}\` without another \`Main\`.`);
                }
            }
            await AppDataSource.manager.update(ActiveToons, { DiscordId: discordId, Name: name }, { Status: status });
            await interaction.reply(`:white_check_mark: \`${name}\`'s status has been changed to \`${status}\`.`);
            if (status === 'Bot') {
