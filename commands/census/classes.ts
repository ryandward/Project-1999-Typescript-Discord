  ...
  if (focusedOption.name === 'class') {
    try {
      ...
      const filtered = choices.filter((choice) =>
        _.lowerCase(choice).startsWith(_.lowerCase(focusedOption.value)),
      );
...
export async function execute(interaction: CommandInteraction) {
  ...
  const query = options.get('class')?.value as string;
  await interaction.reply(
    `https://discordjs.guide/search?q=${encodeURIComponent(query)}`,
  );
}
